...
#define TAG "ti.intent.parseuri"

using namespace v8;

static Persistent<Object> bindingCache;

static void TiIntentParseuri_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiIntentParseuri.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::TiIntentParseuriBindings::lookupGeneratedInit(
...
static void TiIntentParseuri_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TiIntentParseuri_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TiIntentParseuri_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiIntentParseuriBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiIntentParseuriBinding = {
	"ti.intent.parseuri",
	TiIntentParseuri_init,
	TiIntentParseuri_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_intent_parseuri_TiIntentParseuriBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.intent.parseuri", &TiIntentParseuriBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiIntentParseuriBindings::lookupGeneratedInit));
}
