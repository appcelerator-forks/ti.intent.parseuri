...
static Handle<Value> TiIntentParseuri_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TiIntentParseuri.getBinding requires 1 argument: binding")));
	}
...
static void TiIntentParseuri_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TiIntentParseuri_getBinding)->GetFunction());
}

static void TiIntentParseuri_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
